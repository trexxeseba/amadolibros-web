...
## QW2 — cierre de eficiencia y bloqueo real (2026-09-07, PR #329)

- Responsable: Codex. Esfuerzo: M. Rama codex/image-throughput desde main d380374775db7b5d2ef80b09ae97351ccdcd88f9. Head 7b02ff50e125edd04dacfd58fc6fabeb4dd8bab5. Reutilización de fuentes nativas exactas y hasta tres tandas secuenciales por cron implementadas; caducidad, mejor master, asociaciones por ficha y checkpoints conservados.
- Prueba real: 4 ejecuciones, 300 referencias cada una, en 104,956–134,839 segundos por ejecución. 1.200 referencias distintas verificadas por SHA y dimensiones, 0 errores de fuente/validación. 916 aptas para Google; 284 siguen por debajo del mínimo nativo. Son copias nuevas en R2 Preview: 0 upgrades de masters existentes, ninguna corrección productiva atribuida a esta prueba.
- Solicitudes: 2.231 reales frente a 2.398 calculadas con el algoritmo anterior sobre las mismas referencias, 167 menos (6,96%). Primeros dos lotes con fuentes compartidas: 16% y 11,87%; siguientes dos sin fuentes compartidas: 0%. No es un benchmark temporal contra la versión anterior.
- CI 34148650388 y Preview completo 34148650383: success en el head actual. El control general de Preview ya se repitió y pasó tras el 503 de imagen observado en una corrida anterior.
- **SEO de producción fallido; NO LISTO PARA MERGE.** Run 34148650498, job 101826024068: /catalogo?page=2 respondió HTTP 503, Cloudflare **1102 / Worker exceeded resource limits**, el 2026-09-07 17:41:58 UTC, Ray a37781d46b7b5a4c-IAD. Ya no corresponde describirlo como un simple fallo de red. Main continúa en d380374 y no recibió este código.
- El índice de imágenes de 51.682.491 bytes observado previamente es un posible contribuyente, no una causa demostrada. El error HTTP no distingue CPU/memoria. Siguiente acción: inspección read-only de logs y límite concreto en Cloudflare; acceso a la integración sugerido a Seba. No elevar límites ni atribuir una causa sin evidencia.
- El auditor registra detalles y sigue fallando: no se debilitó la validación. 52 pruebas focales y registro local validate-ci 1.668/1.668 con ambos builds. Producción todavía no usa la aceleración.
- Evidencia durable: docs/evidence/qw2-efficiency-close-2026-09-07.json y qw2-efficiency-1200-rows.json.gz, recuperación gzip idéntica y SHA del contenido registrados. Corridas 34146777648 / 34147238152 / 34147760424 / 34148650383.
- Sin merge ni deploy de producción; sin escritura en R2 productivo, cambios de Merchant o checkout. QW2 continúa dentro de la única Gran Apuesta Merchant. #329 queda Draft, pendiente de resolver el bloqueo y de aprobación explícita de Seba.
