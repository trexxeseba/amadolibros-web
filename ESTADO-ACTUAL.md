...
- Lote de conexiones: Seba pidió iniciar con los conectores y comunicó que no puede instalar Cloudflare. Drive permite leer GA4 CONTROL, actualizado al 4/9; faltan fechas/filtro para usarlo como informe actual. Se preparó diagnóstico de sólo lectura vía las credenciales existentes de GitHub Actions, con GA4, D1 Preview/Production y catálogo independientes. 15/15 tests locales; ejecución remota todavía pendiente. No requiere instalar el plugin. Identidad de GitHub y propiedad de este repositorio verificadas como `trexxeseba`; main `0ae0a50` incorporado a la rama conservando el registro del incidente de portadas.
...
- Primera entrega implementada: `/admin` protegido por JWT verificado de Cloudflare Access + audiencia, hostname y correos explícitos, desactivado por defecto; consultas de pedidos/correos sin PII; catálogo y estado de actualización; lector/exportador de GA4 exclusivo de la web. Navegación por seis secciones y períodos 7/30 días.
- Evidencia local: 10 tests focales aprobados; registro de validación compartida termina en `Validación completa OK`, 1.657 tests aprobados / 0 fallos y builds checkout OFF/ON completados. El wrapper no entregó código de salida por cancelación de aprobación de red; esta limitación queda registrada en `docs/admin-web/IMPLEMENTACION.md`.
- Vista de revisión: la primera entrega `docs/admin-web/AMADO-panel-revision.html` tenía un defecto de navegación confirmado por Seba (BlobNotFound al abrir Productos). Se retira esa salida generada del repositorio y se reemplaza por el archivo de entrega `AMADO-panel-revision-corregido.html`, generado con `scripts/admin-web-review.mjs`. Las fuentes continúan sin conectar; no se inventaron métricas ni se diagnosticó una caída de la tienda.
- **Estado: implementado y probado localmente, pendiente de conexiones y verificación en Preview. No terminado ni operativo en producción.** No se activó Access, no se agregaron bindings, no hubo migraciones, compras, correos, escritura a datos del negocio, merge ni deploy de producción.
- Próximo lote: conexión durable de GA4, verificación de acceso privado y navegador; luego revisión de Seba sobre la versión concreta. Edición de banners/contenido registrada para más adelante.
- Guardado remoto pendiente: la revisión automática bloqueó el push de la rama a `trexxeseba/amadolibros-web`, por autorización del destino/confianza no establecida. No se reintentó por otra vía ni se abrió PR. Se solicita aprobación de Seba sobre el código ya preparado. Compilación específica Wrangler/Pages pendiente por paquete ausente de la caché offline; no afecta la evidencia de los dos builds Astro ni de los tests.
- Corrección de la revisión (2026-09-08): enlaces internos por fragmento y selección CSS, sin dependencia de scripts ni rutas `/admin` fuera del archivo. Se agrega regresión para los 96 enlaces, 12 vistas y destinos únicos. La reparación no modifica la aplicación productiva ni intenta el push bloqueado. La vista generada es un entregable separado; el generador y la corrección permanecen en la rama local.
