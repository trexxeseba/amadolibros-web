...
- Causa de A07 identificada tras nueva captura: `redirect: 'error'` no está implementado en workerd. Reproducción local devuelve el TypeError exacto antes de conectar a certificados. Se corrige a `manual` + rechazo de respuestas no exitosas en autenticación, catálogo y metadatos. La prueba anterior sustituía fetch dentro del Worker y ocultaba este error; ahora ejecuta fetch real de workerd con destinos sintéticos controlados, incluyendo redirects rechazados. Ajuste pendiente de publicar y verificar; ingreso real de Seba aún no confirmado.
