...
- Estado: PR #333 IMPLEMENTADO Y VERIFICADO EN CLOUDFLARE AISLADO, head de código `c4007f3`. Corrida `34215546456`: 57.208/57.208 referencias conservadas; 3.697/3.697 ofertas con XML idéntico; 154/154 comprobaciones de imágenes y 4/4 páginas, cero fallos. De las 154 imágenes, cinco son la comparación anterior; las 149 peticiones de la ruta nueva tuvieron cero GET del manifiesto global y cero fallback.
...
- Correcciones del ensayo: una preparación inicial devolvió 1102; se dejó completar el stream del snapshot antes de preparar y las dos siguientes inicializaciones completas terminaron bien. No se capturó la causa CPU/memoria original. El observador perdió el helper de nombres al empaquetarse: corregido y protegido con ejecución del bundle real. La prueba de empaquetado ahora corre después del `npm ci` de Astro, mientras la prueba nativa sigue sin dependencias. Validación local completa y ambos builds verdes; evidencia por portada y tiempos en `docs/evidence/cover-scroll-2026-09-08.json`, original comprimido reversible y dos capturas en la misma carpeta.
