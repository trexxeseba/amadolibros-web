...
- PR #330 fusionado por squash en `0ae0a50e8c9da74a023e4e609966d87ab3e72aa6`. Despliegue publicado a las 23:47:28 UTC: [run 34171140241](https://github.com/trexxeseba/amadolibros-web/actions/runs/34171140241), validación, Deploy, smoke y catálogo pausado **success**. Worker Sync: run 34171140269 **success**.
- Causa corregida: el lector cargaba un manifiesto de imágenes de más de 60 MB para páginas y portadas, incluidas URLs inmutables. Las URLs por SHA leen directamente el objeto R2; catálogo, ficha, proxy estable y feed recorren el manifiesto por streaming. Una página agrupa su selección y la caché guarda sólo los punteros públicos necesarios.
- Criterio de aceptación cumplido en la web pública: inicio y tres vistas de catálogo **4/4 HTTP 200**; **104/104 imágenes HTTP 200**, tipo de imagen correcto, decodificación válida y SHA esperado, **0 fallos**. Incluye las cuatro portadas de la captura: MLU634431651, MLU709390092, MLU690771648 y MLU679987262.
- Navegador: catálogo visible con portadas; las primeras **12/12 variantes responsive de Cloudflare** se decodifican correctamente. Las otras 36 imágenes de esa página seguían pendientes de carga diferida en esa observación; no se cuentan como verificadas en navegador. Sus URLs originales están cubiertas por la comprobación HTTP.
- Reejecución del control comercial: run 34165334287, intento 2, paso Audit protected Preview **pass / 0 críticos**: 80/80 páginas de Preview y **80/80 imágenes de producción**, todas con mínimo 500 px; feed de Preview 3.695 ítems. Las imágenes que antes devolvían 503 superaron este control.
- **El workflow amplio de Preview sigue fallido**: el paso de enriquecimiento recibió HTTP 404 en `/libro/MLU801608158`; causa sin establecer. El informe del hub no se generó por el corte anterior. No se modifica ni debilita esa compuerta y no se afirma que todos los checks estén verdes.
- Evidencia reproducible por URL, bytes, SHA, dimensiones, tiempos y alcance en `docs/evidence/cover-production-2026-09-07.json`. Ensayo previo y snapshot referenciados en `docs/evidence/cover-resource-2026-09-07.json`.
- Límite: esta recuperación de disponibilidad no significa que toda imagen del catálogo haya sido censada ni que QW2 de calidad esté terminado. Sigue pendiente conseguir fuentes mejores para los masters pequeños y un índice público persistente para acelerar lecturas frías. En esta observación el servidor del catálogo informó ~0,39–0,49 s con selección en caché y 3,80 s en página 2; los tiempos completos del cliente de verificación fueron 9–17 s y no representan la experiencia en Uruguay.
- Próxima acción de imágenes: índice público pequeño. Responsable Codex; esfuerzo M; aceptación: páginas frías sin escanear el manifiesto global, portadas estables e inmutables verificadas, feed sin cambios sobre el mismo snapshot. PR #329 sigue separado y sin fusionar.
- Incidencia de validación a resolver: MLU801608158 en Preview. Responsable Codex para verificar origen del 404, coordinación con la línea de enriquecimiento de Claude; esfuerzo S; aceptación: distinguir catálogo/entorno/publicación con evidencia por ID y recuperar el gate completo sin ocultar el 404. No se envió una nueva tarea a Claude en esta sesión.
- Sin modificaciones al código de checkout, precios ni reglas Merchant. Se ejecutó el despliegue habitual; D1 informó que no había migraciones pendientes. No se realizaron escrituras manuales en masters o manifiestos.
