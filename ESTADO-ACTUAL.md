...
## Portadas: recuperación publicada; aceleración en validación — 2026-09-08

- Responsable: Codex. Esfuerzo: M. Trabajo operativo dentro de Google Merchant Center, única Gran Apuesta activa.
- Recuperación #330: fusionada con aprobación de Seba en `0ae0a50`, desplegada por la corrida `34171140241`. Verificación pública: 4/4 páginas y 104/104 imágenes válidas. Esto corrigió los HTTP 503/Cloudflare 1102; no completó la optimización de velocidad.
- Problema restante: el proxy estable y la selección de portadas todavía recorren el manifiesto completo de más de 60 MB en lecturas frías. El ensayo anterior midió 5,45–8,40 s para ese proxy. La captura posterior corresponde a MLU1094451174, en `/libros/psicologia`; al inspeccionarla después cargaba. No se atribuye una duración exacta al intervalo de la captura.
- Rama nueva `codex/cover-public-index`, desde main actualizado `0ae0a50`: índice público persistente dividido en 256 shards, sin historial privado. Cubre todas las entradas existentes desde la primera sincronización; no requiere una lista manual de productos. Las URLs de la web siguen funcionando.
- Integridad: shards y root inmutables por SHA; el puntero se publica en los metadatos del mismo PUT condicional que el manifiesto. Un conflicto exige releer, combinar y reconstruir. Se conservan origen, MIME, integridad, masters y comprobaciones vigentes. El feed no cambia de lector.
- Continuidad: si el índice falta o está dañado se usa la lectura anterior, registrando explícitamente el fallback. Una reconstrucción diaria automática repara los objetos derivados; no depende de que una imagen necesite actualizarse.
- Aceptación: pruebas de concurrencia y publicación incompleta, ausencia de GET del manifiesto en lecturas sanas, todas las entradas preservadas y XML Merchant idéntico. Ensayo en Worker Cloudflare temporal sobre un snapshot productivo congelado; escrituras sólo en un prefijo aislado del bucket de Preview. Comparación fría antes/después, portadas de ambas capturas, catálogo y categoría Psicología. Objetivo medido: mediana del handler al menos 50% menor y menor a 1.500 ms.
- Estado: PR #333 IMPLEMENTADO; lectura y scroll verificados en Cloudflare aislado, reconstrucción repetida en corrección por memoria. Aceptación inicial sobre `c4007f3`, corrida `34215546456`: 57.208/57.208 referencias conservadas; 3.697/3.697 ofertas con XML idéntico; 154/154 comprobaciones de imágenes y 4/4 páginas, cero fallos. De las 154 imágenes, cinco son la comparación anterior; las 149 peticiones de la ruta nueva tuvieron cero GET del manifiesto global y cero fallback. Ese éxito inicial no valida por sí solo la reconstrucción repetida.
- Velocidad medida en el handler del Worker temporal con caché fría aislada por corrida, mismo snapshot: mediana 3.984 → 595 ms (−85,07%). MLU1094451174, la portada de la captura: 3.984 → 576 ms, bytes idénticos. Las tres vistas de catálogo tardaron 1.199–1.970 ms. No es una medición de navegador desde Uruguay ni una mejora productiva ya publicada.
- Validación: 27 pruebas nuevas de integridad, CAS, fallos de carga, reparación diaria y header honesto incluso con imagen cacheada. Suite 1.684/1.684 y ambos builds verdes en `34215546368`; Preview completo verde en `34215546331`; ensayo anterior de recuperación también verde en `34215546427`. La primera corrida de este índice falló por un asset omitido en el runner de categorías; fue corregido y repetido, sin debilitar la aceptación.
- Evidencia permanente por URL, bytes, SHA, lectura y tiempos: `docs/evidence/cover-index-2026-09-08.json`. Snapshot íntegro y HTML en artifact `10051662113` (90 días); resumen de la corrida conservado. Eliminados los 260 objetos temporales y el Worker del ensayo, ambos con confirmación exitosa. Cero escrituras en datos productivos.
- Scroll real añadido en `34224677948`, head QA `b972d09`: índice completo de 58.864 referencias sobre 78.003.033 bytes; 48/48 portadas de Psicología entraron en pantalla y terminaron decodificadas, HTTP 200, cero errores y cero comprobaciones pendientes. Las 48 respuestas iniciales registraron `public-index`, cero GET del manifiesto global y ninguna cabecera de medición ausente.
- Velocidad de navegador: el primer recorrido (teclado + rueda, pausas y regreso para cubrir filas saltadas) tuvo 24 portadas con espera visible; máximo 3.992,5 ms. No fue un benchmark de scroll continuo. En la segunda visita, con las 48 imágenes en caché del navegador, las 44 portadas posteriores a la primera fila estuvieron listas antes de entrar; las cuatro iniciales registraron hasta 47,5 ms hasta confirmar `decode()`. Cero errores en ambas visitas. La espera mide exposición antes de decodificar, no garantiza el instante de pintura.
- Alcance: Chrome remoto, 1363×936; renderer, `loading`, `srcset` y `sizes` reales. El ensayo usa Cloudflare Images nativo con un Worker aislado, diferente de la entrega `/cdn-cgi/image` productiva. Eliminar el índice grande quedó demostrado; la latencia fría residual todavía existe en este ensayo y no se presenta como resuelta ni como un tiempo medido desde Uruguay.
- Correcciones del ensayo: el observador perdió el helper de nombres al empaquetarse: corregido y protegido con ejecución del bundle real. La prueba de empaquetado ahora corre después del `npm ci` de Astro, mientras la prueba nativa sigue sin dependencias. Evidencia por portada y tiempos en `docs/evidence/cover-scroll-2026-09-08.json`, original comprimido reversible y dos capturas en la misma carpeta.
- Bloqueo detectado al repetir la reconstrucción: las inicializaciones intermedias exitosas no resolvieron el 1102. En `34227121174`, snapshot de 78.432.808 bytes y 59.164 entradas, primera reconstrucción completa OK; segunda `exceededMemory` durante el PUT condicional del manifiesto, confirmado por Cloudflare. La escritura por bloques sola no bastó. Se agrega lectura incremental completa además de escritura acotada, preservando todos los historiales privados y metadatos. Comparación local del lector contra JSON.parse del snapshot real: igualdad completa de las 59.164 entradas. Compuerta real: dos reconstrucciones completas consecutivas y comparación íntegra de lo escrito; cualquier fallo reprueba, sin reintentos para ocultarlo. Sin cambios en datos productivos.
- La lectura incremental más escritura acotada ya pasó dos reconstrucciones completas en `34228477299` (`04257bc`): 59.433 entradas, 78.801.346 bytes, documento privado íntegro; 154/154 imágenes, 4/4 páginas y 3.697 ofertas idénticas. Mediana del handler 5.829 → 840 ms (−85,59%). Trazas de ambas reconstrucciones `ok`; 260 objetos y Worker eliminados. Falta la aceptación del reintento con conflicto nativo de R2. Evidencia y fallo anterior conservados en `docs/evidence/cover-index-resource-2026-09-08.json`.
- Limpieza del ensayo de scroll: 260 objetos del prefijo temporal y Worker eliminados con respuesta exitosa; artefacto `10055524508`, cero escrituras productivas. Las trazas de la corrida final registraron resultados `ok`, sin excepciones. Responsable Codex, esfuerzo M; criterio del índice cumplido, velocidad fría pendiente de evaluación en la entrega productiva después de autorización.
- Producción: pendiente aprobación expresa de Seba para merge y despliegue de #333. Tras aprobar, esperar los deploys de Pages y Worker Sync y su sincronización; comprobar `x-cover-index: public-index` en las portadas estables, HTTP 200/bytes y páginas en la web pública. El pipeline inicializa todas las referencias existentes y continúa automáticamente con las nuevas. No declarar terminado en producción hasta esa comprobación. Checkout intacto.
