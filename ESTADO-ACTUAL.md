...
- **Prioridad actual: detectar fallas.** Revisión CTO realizada por agente de IA sobre Pages, Worker sync, R2, D1 y panel. Lote preparado: `/api/status` leído desde Resumen/Funcionamiento, diagnósticos 503 conservados, fecha y cobertura visibles, fotos/banners explícitamente pendientes de conexión. Propuesta mínima: Checkly externo + Sentry para errores, webhooks hacia receptor privado separado. No hay cuentas nuevas ni sensores activos. 31 pruebas focalizadas correctas; ver `docs/admin-web/MONITOREO.md`. Publicación y comprobación remota de este lote pendientes.
- **Ingreso confirmado por Seba:** después de corregir A07 respondió «AHORA SI ME ENCANTÓ ENRIQUEZCÁMOSLO». Se prepara la ampliación de la misma revisión privada: visitas diarias, comparación con período previo completo, fichas más vistas, barras por canal/dispositivo/compra y avisos con acciones. 29 pruebas focalizadas correctas. Pendiente comprobar los informes enriquecidos reales y publicar el lote.
- Causa de A07 identificada y corregida: `redirect: 'error'` no está implementado en workerd. Reproducción local devuelve el TypeError exacto antes de conectar a certificados. Cambiado a `manual` + rechazo de respuestas no exitosas en autenticación, catálogo y metadatos. La prueba anterior sustituía fetch dentro del Worker y ocultaba este error; ahora ejecuta fetch real de workerd con destinos sintéticos controlados, incluyendo redirects rechazados. Publicado commit `b5437d5`, [run 34282927166](https://github.com/trexxeseba/amadolibros-web/actions/runs/34282927166) **success**: 24 pruebas, 14 escenarios de runtime y 3 redirecciones privadas. Worker `df559c70-ae93-4b22-a18c-247e3f6b386d`; GA4 actualizado 21:52:31 UTC. Pendiente únicamente confirmar reingreso real de Seba para esta incidencia; no se fusionó main.
...
- Revisión publicada: https://amadolibros-admin-preview.undiaes.workers.dev/admin. Worker dedicado `amadolibros-admin-preview`, aislado de Pages y sincronizador. Cloudflare Access con proveedor onetimepin existente y política exclusiva del titular; verificación adicional del JWT en el servidor. El primer ingreso real del titular aún debe confirmarlo Seba.
