...
## ADMIN-WEB-01 — revisión privada y monitoreo conectados (2026-09-09)

- **Ampliación de monitoreo detenida; foco en imágenes:** Seba indicó continuar con las imágenes y evitar duplicar lo avanzado en otro chat. PR #336 pasó CI y Preview 34359782792: ficha MLU651526046 y 80 páginas auditadas correctas. El intento Checkly 34359921633 falló con HTTP 403 al intentar la ampliación y dejó pausados los tres controles existentes; diagnóstico 34360210577 confirmó la pausa y que no se creó el cuarto control. Esta revisión restaura únicamente los tres controles previos. Prueba remota de restauración pendiente al registrar este avance. El soporte de google_imagen permanece preparado pero sin control activo; no se declara cobertura automática de Product.image.

- **Checkly conectado y activo:** Seba guardó la clave y confirmó «LISTO». Acceso verificado en su cuenta trial. [Conexión real 34300501013](https://github.com/trexxeseba/amadolibros-web/actions/runs/34300501013), commit `00ec13a8`, **success**: evento de falla firmado recibido 01:46:55 UTC y recuperación 01:47:07 UTC, ambos correlacionados con el resultado real del proveedor. Fixture desactivado y ruta retirada de producción. Tres monitores de lectura activos: catálogo/sincronización y página del catálogo cada 10 minutos; imágenes y navegación cada 120 minutos. Canal exclusivo hacia el backend, sin suscripciones automáticas ni destinatarios personales.
- **Panel y registro verificados:** [verificación 34299820288](https://github.com/trexxeseba/amadolibros-web/actions/runs/34299820288), reintento del 9 de septiembre 01:49:30 UTC, **success**. Bindings privados, D1 separada de pedidos, lectores reales y secciones HTML comprobados. Dos controles correctos y uno con falla registrada. Worker privado `04e88aae-14ee-4d3c-b748-3a9446df58da`, [run 34300501017](https://github.com/trexxeseba/amadolibros-web/actions/runs/34300501017) **success**: 38 pruebas focales, 16 escenarios workerd, 11 escenarios Chromium/D1 efímero y tres accesos anónimos/falsos rechazados. GA4 actualizado 01:48:30 UTC.
- **Primer hallazgo del monitor:** la recorrida de navegador del 9 de septiembre 01:47:15 UTC falló por `IMAGEN_VISIBLE_NO_CARGA`: hubo imágenes visibles que no estuvieron listas dentro del plazo de 8 segundos. La alerta está guardada y se refleja en el panel. Esto no demuestra por sí solo una imagen permanentemente rota ni identifica todavía cada recurso afectado. [Diagnóstico acotado 34300731264](https://github.com/trexxeseba/amadolibros-web/actions/runs/34300731264). Catálogo y sincronización respondieron correctamente en sus comprobaciones.
- **Límites:** muestra por navegador de inicio, catálogo y una ficha, con algunas imágenes de fondo visibles; no todas las fotos ni todos los dispositivos, sin prueba de pago. Cada monitor muestra fecha y frecuencia, y distingue atraso, pausa o falta de resultados. No hubo merge, despliegue de la tienda, escrituras de negocio ni cambio de plan. El calendario horario de GA4 continúa pendiente de merge autorizado.

### Historial previo a la conexión real

- **Cuenta Checkly creada:** Seba adjuntó pantalla autenticada de la cuenta. La captura indica Team Trial y descenso a Hobby el 22 de septiembre; no prueba un cobro. Se prepara diagnóstico GET de la cuenta exacta, sin escritura y sin mostrar payloads. En este punto aún faltaba guardar `CHECKLY_API_KEY`; quedó resuelto el 9 de septiembre como se registra arriba. Las claves de usuario se crean en User Settings → API keys; las claves de servicio son Enterprise y no se requieren. No se pidió ni recibió la clave en la conversación.
- **Ensayo completo verificado:** commit `390c4104`, [run privado 34288960290](https://github.com/trexxeseba/amadolibros-web/actions/runs/34288960290) success: 34 pruebas focalizadas, 16 escenarios workerd de acceso/fuentes y 11 escenarios del circuito Chromium→receptor workerd→D1 efímero→panel. Foto rota, HTTP 200 inválido, demora visible, fallback, lazy, error API, firma, duplicados, aviso tardío y recuperación comprobados. Datos completamente sintéticos; no son incidentes de la tienda ni avisos entregados por Checkly. Worker privado actualizado a `e1ba50b7-610d-4ab6-828a-c37e11085570`; GA4 23:05:09 UTC; tres accesos anónimos/falsos continúan redirigiendo al login. [CI compartido 34288963083](https://github.com/trexxeseba/amadolibros-web/actions/runs/34288963083) success. Receptor y base remota aún sin provisionar; pendiente cuenta Checkly, configuración y prueba real del proveedor.
- **Circuito de incidentes preparado (continuación 2026-09-08):** receptor Checkly con HMAC, ocho transiciones oficiales, deduplicación y protección de orden temporal. Historial en esquema exclusivo; fallas abiertas no vencen. Sensor para imágenes rotas, inválidas HTTP 200 y visibles demoradas; distingue fallback de recuperación. Ensayo Chromium→workerd→D1 efímero→panel preparado en CI, todavía pendiente de ejecutar en este punto. No hay receptor publicado, nueva base remota ni cuenta Checkly conectada. El panel puede representar el registro al conectar su binding; mientras tanto indica pendiente. Última versión previamente publicada: `2257e483`, [run 34286681060](https://github.com/trexxeseba/amadolibros-web/actions/runs/34286681060), success.
- **Prioridad actual: detectar fallas.** Revisión CTO realizada por agente de IA sobre Pages, Worker sync, R2, D1 y panel. Lote preparado: `/api/status` leído desde Resumen/Funcionamiento, diagnósticos 503 conservados, fecha y cobertura visibles, fotos/banners explícitamente pendientes de conexión. Propuesta mínima: Checkly externo + Sentry para errores, webhooks hacia receptor privado separado. No hay cuentas nuevas ni sensores activos. 31 pruebas focalizadas correctas; ver `docs/admin-web/MONITOREO.md`. Publicado en la misma revisión privada: commit `37c69dfd`, [run 34286404858](https://github.com/trexxeseba/amadolibros-web/actions/runs/34286404858) success: 31 pruebas, 16 escenarios workerd, fuente real `/api/status` ok (22:32:30 UTC), GA4 enriquecido validado y 3 comprobaciones de acceso privado. Worker `3b1d3ae3-5979-45d5-bc5d-5d72099a5d4f`. CI compartido [34286409625](https://github.com/trexxeseba/amadolibros-web/actions/runs/34286409625): 1.689 tests, cero fallos y ambos builds. Revisión CTO final sin bloqueantes; ajuste adicional de normalización ISO preparado para descartar comentarios en fechas.
- **Ingreso confirmado por Seba:** después de corregir A07 respondió «AHORA SI ME ENCANTÓ ENRIQUEZCÁMOSLO». Se prepara la ampliación de la misma revisión privada: visitas diarias, comparación con período previo completo, fichas más vistas, barras por canal/dispositivo/compra y avisos con acciones. Informes enriquecidos reales de 7/30 días comprobados y publicados; 31 pruebas focalizadas correctas.
- Causa de A07 identificada y corregida: `redirect: 'error'` no está implementado en workerd. Reproducción local devuelve el TypeError exacto antes de conectar a certificados. Cambiado a `manual` + rechazo de respuestas no exitosas en autenticación, catálogo y metadatos. La prueba anterior sustituía fetch dentro del Worker y ocultaba este error; ahora ejecuta fetch real de workerd con destinos sintéticos controlados, incluyendo redirects rechazados. Publicado commit `b5437d5`, [run 34282927166](https://github.com/trexxeseba/amadolibros-web/actions/runs/34282927166) **success**: 24 pruebas, 14 escenarios de runtime y 3 redirecciones privadas. Worker `df559c70-ae93-4b22-a18c-247e3f6b386d`; GA4 actualizado 21:52:31 UTC. Reingreso confirmado posteriormente por Seba; incidencia cerrada, sin merge de main.
- Incidencia de ingreso: Seba recibe el 403 del panel después de llegar al enlace privado. Ajuste publicado en el mismo enlace, commit `d56c04d`, [run 34282206299](https://github.com/trexxeseba/amadolibros-web/actions/runs/34282206299) **success**: cookie de aplicación firmada como alternativa cuando falta la cabecera Access; se mantienen firma, emisor, audiencia, vigencia y titular. Rechazo con reingreso y referencia cerrada sin datos de sesión. 24 pruebas + 9 escenarios workerd + 3 redirecciones privadas, todos correctos. Worker `9cbf9402-6649-4224-9668-9c174747aede`; GA4 actualizado 21:44:17 UTC. Causa exacta y primer ingreso exitoso aún no confirmados; pendiente reintento de Seba. Las comprobaciones anónimas anteriores no cubrían el acceso real del titular.
- Encargo de Seba: panel exclusivo de amadolibros.com con visitas, compra, problemas registrados, pedidos, productos y estado. Primera entrega de sólo consulta; banners/contenido editables quedan para un lote posterior. No reemplaza la Gran Apuesta Merchant.
- Revisión publicada: https://amadolibros-admin-preview.undiaes.workers.dev/admin. Worker dedicado `amadolibros-admin-preview`, aislado de Pages y sincronizador. Cloudflare Access con proveedor onetimepin existente y política exclusiva del titular; verificación adicional del JWT en el servidor. Seba confirmó su ingreso real después de corregir A07.
- Datos: GA4 7/30 días cerrados, sólo los dominios web; pedidos y correos productivos con SELECT acotados sin PII; catálogo/metadatos de la web. La interfaz aclara revisión con datos reales. Ninguna escritura a pedidos, pagos o catálogo.
- Persistencia: ambos períodos de Analytics en un valor v2 del KV propio del panel. Snapshot actualizado y releído el 2026-09-08 21:24:36 UTC. Una falla preserva el valor anterior y el panel no convierte ausencia de datos en cero.
- Evidencia principal: [run privado 34280416597](https://github.com/trexxeseba/amadolibros-web/actions/runs/34280416597), commit `81f9c37`, **success**: 22/22 pruebas, refresh verificado, Access, despliegue del Worker y tres peticiones reales redirigidas al login, incluyendo JSON e identidad falsa. Versión Worker `aee64b61-6129-4745-afac-3b3531f069bf`.
- CI compartido sobre ese código: [34280483216](https://github.com/trexxeseba/amadolibros-web/actions/runs/34280483216), **success**, 1.680 tests / 0 fallos y ambos builds. El primer despliegue privado devolvió 404 inmediatamente después de publicar; la propagación se confirmó con lecturas posteriores y se agregó reintento acotado. No se ocultó ese fallo histórico.
- [PR #335](https://github.com/trexxeseba/amadolibros-web/pull/335) en borrador, rama `codex/admin-web-observability`, main `1aaaba8` incorporado. Auditoría general de Preview debe consultarse en el PR; no se declara aprobada aquí. Guardado remoto por conector autenticado resuelto tras verificar identidad/propiedad del repo; shell Git no tiene credencial de escritura.
- **Pendiente: aprobación de Seba para fusionar.** El calendario horario, minuto 17, está implementado en `admin-web-refresh.yml` pero no se ejecuta desde una rama de revisión. El merge lo habilita y también dispara el deploy habitual de Pages. No se fusionó ni se publicó la tienda; su ruta `/admin` sigue apagada sin variables explícitas. La revisión privada ya publicada se puede usar con los datos extraídos; pedidos/catálogo se consultan al abrir cada vista.
- Documentación completa en `docs/admin-web/IMPLEMENTACION.md`. El HTML descargable aprobado sigue como artefacto de diseño; para usar datos reales se debe abrir la revisión privada.
...
- Estado: PR #330 implementado y verificado en Cloudflare con datos productivos de sólo lectura: run 34164760251, código 285fb457. Tres vistas de catálogo HTTP 200; 104/104 imágenes válidas por SHA, incluidas las cuatro de la captura y 100 URLs inmutables sin lectura de manifiesto. Feed 3.695/3.695 con XML de ítems idéntico sobre el mismo snapshot. CI 34164760183 verde; suite 1.657/1.657 y ambos builds.
- El audit general de Preview pasó 80/80 fichas, pero falló en 49/80 imágenes porque sigue pidiendo las URLs de producción sin este arreglo. Se conserva el fallo, no se afirma que todos los checks estén verdes.
- Límite de esta contención: catálogo frío 7,9–9,1 s; selección en caché 0,664 s. Proxy estable frío 5,45–8,40 s. Imágenes inmutables: mediana 226,5 ms y máximo 623 ms. Falta un índice público pequeño y persistente para la velocidad de las lecturas frías.
- Evidencia por URL y límites en docs/evidence/cover-resource-2026-09-07.json. Snapshot original en artifact 10033801752, retención 90 días. Worker temporal eliminado. Producción todavía no está restaurada por este PR: aprobación explícita, despliegue y verificación pública pendientes. Sin cambios en checkout, precios, Merchant ni escrituras de datos productivos.
