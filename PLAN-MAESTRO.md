...
## ADMIN-WEB-01 — panel exclusivo de la web (autorizado 2026-09-07)

- **Alcance corregido por Seba 2026-09-09:** continuar con las imágenes; el panel avanzó en otro chat. PR #336 conserva la corrección de Product.image, con Preview y auditoría de 80 páginas aprobados. Se detiene la ampliación de monitoreo. El intento de cuarto control devolvió HTTP 403 y pausó los controles existentes; se restaura únicamente la conexión de los tres controles previos, con nueva comprobación de entrega firmada. No se agrega un cron ni otra plataforma; no se fusiona ni despliega la tienda.

- **Conexión Checkly, 2026-09-09:** Seba guardó la clave indicada y confirmó «LISTO». Acceso real verificado en la cuenta de su captura: plan trial, cero monitores previos. Se implementa la conexión pedida en recursos aislados: receptor dedicado, D1 exclusiva, webhook firmado y pruebas de entrega del proveedor antes de habilitar controles de lectura. Sólo el panel privado recibe la conexión; no hay merge, despliegue de la tienda, cambios en Access, escrituras de negocio ni contratación. Frecuencias iniciales compatibles con el presupuesto Hobby: dos API cada 10 minutos y un navegador cada 120 minutos, sin reintentos automáticos. Activación completada con entrega real de falla/recuperación en run 34300501013 y verificación de bindings/lectores/panel en reintento de run 34299820288. Dos controles correctos y una alerta de imágenes visibles que no cargaron en 8 segundos; ver ESTADO-ACTUAL.md.

- **Continuación de Seba «cómo seguimos», 2026-09-08:** preparar el circuito de incidentes y demostrar falla/recuperación de imágenes. Receptor firmado desactivado por defecto; esquema exclusivo probado en base efímera, sensor sólo en fixture sintético y tabla privada con estado pendiente si falta binding. No se crea una base remota, no se publica el receptor ni se instrumenta la tienda. Conexión real Checkly y activación quedan sujetas a cuenta disponible y validación de la entrega concreta.
- **Prioridad expresamente ajustada por Seba el 2026-09-08:** detectar fallas, fotos/banners que no cargan y procesos trancados; investigar plataformas que reporten directamente al panel y realizar revisión de arquitectura con enfoque CTO. Revisión independiente realizada por agente de IA, sin contratar ni contactar personas. Primera ampliación incorpora `/api/status` al panel privado, distingue incidentes de compra de fallas técnicas y explicita cobertura de imágenes todavía pendiente. Diseño e integración posterior en `docs/admin-web/MONITOREO.md`. No se contratan servicios ni se activa instrumentación productiva en este lote.
- **Enriquecimiento autorizado 2026-09-08:** Seba confirmó su ingreso y pidió enriquecer el panel. Lote acotado sobre la misma revisión privada: comparación de visitas con período previo, evolución diaria, ranking de fichas vistas, actividad de compra visual y avisos accionables. Se conserva modo consulta, diseño aprobado y las fuentes existentes. La aprobación del acceso no constituye aprobación de merge/productivo.
- **Encargo vigente de Seba:** administrar y observar únicamente amadolibros.com: visitas, interés en productos, recorrido de compra, errores confirmados, pedidos, catálogo y banners. WooCommerce es una referencia de uso; no se pide una migración ni un panel multicanal.
- **Responsable:** Codex. **Esfuerzo:** M para la primera entrega de consulta; conexiones y edición se verifican por separado.
- **Continuación autorizada 2026-09-08:** preparar persistencia y actualización automática de GA4, verificar Cloudflare Access y entregar una revisión privada conectada, alojada en un Worker dedicado con acceso exclusivo al titular. Recursos del panel separados del negocio. La activación productiva y el merge siguen sujetos a aprobación de la versión concreta.
- **Lote de conexiones autorizado 2026-09-08:** Seba pide empezar usando conectores y comunica que no puede instalar Cloudflare. Se comprueban las fuentes de Google Drive y se prepara un diagnóstico de sólo lectura mediante las credenciales existentes de GitHub Actions; sin activación, cambios de permisos, datos públicos del negocio ni despliegue. La escritura periódica del snapshot queda pendiente de validar este acceso y su destino privado.
- **Estado:** revisión privada conectada publicada y verificada (run 34280416597, PR #335 en borrador). Persistencia de GA4 operativa; calendario horario preparado y pendiente de merge aprobado. La ruta /admin de la tienda sigue desactivada. Es una acción acotada autorizada expresamente; no se abre una segunda Gran Apuesta ni se reordena Merchant.
- **Alcance inmediato:** panel protegido y desactivado por defecto, lectura de pedidos existentes sin datos personales, catálogo y estado de actualización; lectura de informes agregados de GA4 con período, fuente y fecha explícitos. Ninguna métrica ausente se convierte en cero. Abandono y error confirmado son conceptos distintos.
- **Criterio de aceptación de esta entrega:** acceso anónimo y credenciales inválidas rechazados antes de consultar datos; ninguna operación de escritura de negocio; fallas parciales visibles; filtros acotados; pruebas de consultas, límites y autenticación; propuesta revisable en rama. No se declara el panel operativo hasta verificar el acceso privado y las fuentes en el entorno destino.
- **Evidencia:** `docs/admin-web/IMPLEMENTACION.md`, pruebas `functions/__tests__/admin-web.test.js`, `admin-web-connections.test.js` y [diagnóstico remoto](https://github.com/trexxeseba/amadolibros-web/actions/runs/34222874855). [PR #335](https://github.com/trexxeseba/amadolibros-web/pull/335) en borrador; [revisión privada](https://amadolibros-admin-preview.undiaes.workers.dev/admin) publicada con datos reales.
- **Entregas posteriores registradas:** conexión durable a GA4 ya existente y validación contra la propiedad; publicación privada; edición de contenido, banners y destacados con previsualización e historial. Precio y stock sólo se editarán tras definir coordinación con su fuente actual.
- **Límites de autorización:** este avance no autoriza merge a `main`, despliegue productivo, migraciones, cambios de permisos productivos ni activación del panel en el dominio de la tienda. La revisión privada separada sí corresponde a la continuación autorizada. La aprobación de Seba debe referirse a la versión concreta ya validada. Checkout, home y sincronizador conservan su implementación.
