...
(`functions/api/_env_config.js`). El código quedó **preparado** para operar
en Preview y Production, pero este lote solo se **desplegó y validó en
Preview**. Production sigue sin credenciales LIVE, sin D1 propia y sin
`ALLOWED_HOSTS` — por diseño, hasta 2-N-E2.
...
| `CHECKOUT_ENABLED` | `"true"` | Sin definir todavía | var | No (ausente = deshabilitado) | `POST /api/orders` y `POST /api/preferences` responden `503 checkout_temporarily_unavailable`; webhook, `/api/orders/status` y `/pedido` siguen funcionando | Repo — `wrangler.toml` |
...
| `ORDERS_DB` (binding D1) | `amadolibros-orders-preview` | No existe — **no crear en este lote** | binding D1 | Sí | `503` sin fallback a Preview, memoria u otra base | `wrangler.toml` |
...
- Production: **no existe** todavía. No hay bloque
  `env.production.d1_databases` ni `[[d1_databases]]` de nivel superior.
  Si `ORDERS_DB` está ausente, el handler responde `503` sin ningún
  fallback (a Preview, a memoria, o a otra base).
- Cuando se cree la base de Production (2-N-E2), debe tener su propio
  `database_id`, nunca reutilizar el de Preview.
...
## Riesgos pendientes para 2-N-E2
...
2. **`MP_COLLECTOR_ID` de Production no está confirmado.** El valor
   ...
   (commit `6e3e181`).
3. **No existe D1, Turnstile widget, ni credenciales LIVE de Production.**
   Ninguno de los tres se crea en este lote, por alcance explícito.
4. **`ALLOWED_HOSTS` de Production no está declarado en ningún
   `wrangler.toml`** — el valor en esta tabla es una propuesta para
   cuando se autorice 2-N-E2, no una configuración ya aplicada.
