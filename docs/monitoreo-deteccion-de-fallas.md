...
> ## Nota de rescate — 2026-09-14
>
> Este documento se escribió el 2026-09-08 en la rama `codex/admin-web-observability`,
> dentro del [PR #335](https://github.com/trexxeseba/amadolibros-web/pull/335), que se
> **cerró sin fusionar** el 2026-09-10 por decisión de producto: se dio de baja Radar y
> el panel que queda es `/panel`. Se rescata acá porque el análisis sigue siendo válido y
> porque estaba perdiéndose en una rama muerta.
>
> **Qué quedó realmente andando.** Según la evidencia de este mismo documento
> ([run 34300501013](https://github.com/trexxeseba/amadolibros-web/actions/runs/34300501013),
> 2026-09-09 01:49 UTC), se activaron **tres controles Checkly de sólo lectura**:
> `/api/status` y `/catalogo` cada 10 minutos, y un recorrido de navegador por inicio,
> catálogo y una ficha cada 120 minutos. Más un Worker receptor `amadolibros-web-monitor`
> con su propia D1. La entrega real por webhook quedó probada: falla 01:46:55 y
> recuperación 01:47:07, correlacionadas por ID de resultado.
>
> **Y acá está el problema.** El canal se creó como WEBHOOK exclusivo, *sin destinatarios
> personales*, y la única pantalla que leía esos incidentes era `/admin` — que nunca se
> fusionó. Entonces el monitoreo corre, detecta y guarda en una D1 **que nadie puede
> mirar, y no le avisa a nadie**. De hecho el recorrido del 9/sep ya clasificó un hallazgo
> real —`IMAGEN_VISIBLE_NO_CARGA` + timeout, imágenes visibles que no cargan en 8
> segundos— y ese hallazgo nunca llegó a una persona.
>
> **El arreglo no es recuperar la rama.** Es agregarle a esos tres controles un canal de
> alerta por correo en Checkly, para que el aviso llegue a una casilla en vez de a un
> Worker sin lector. Recuperar el panel `/admin` sería reabrir una decisión ya tomada y
> duplicar `/panel`.
>
> **Lo que no pude verificar** desde esta sesión: si los tres controles siguen activos hoy,
> si el plan `trial` de Checkly venció, y si el Worker receptor sigue desplegado. Eso se
> ve entrando a la cuenta de Checkly y al panel de Cloudflare. Los precios y límites que
> cita el documento se verificaron el 2026-09-08 y pueden haber cambiado.
>
> Debajo, el documento original sin modificar. Sus referencias a `/admin`, a Cloudflare
> Access y a Radar corresponden a ese diseño descartado; el resto —la comparación de
> plataformas, las reglas para detectar una foto rota de verdad y el diseño del receptor—
> sigue en pie.
