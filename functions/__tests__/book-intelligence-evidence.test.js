...
test('una fuente secundaria fuerte no basta para auto-publicar datos de edición', () => {
  const result = classifyBookIntelligenceEvidence(item(), [
    evidence('google_books', {
  ...
  assert.equal(result.edition_fields_auto_publishable.language, false);
});

test('fuente oficial exacta o dos familias coincidentes habilitan el campo', () => {
  const official = classifyBookIntelligenceEvidence(item(), [
    evidence('publisher', { pages: 304, language: 'Español' }),
  ]);
  assert.equal(official.edition_fields_auto_publishable.pages, true);
  assert.equal(official.edition_fields_auto_publishable.language, true);

  const corroborated = classifyBookIntelligenceEvidence(item(), [
    evidence('google_books', { pages: 304, language: 'Español' }),
    evidence('open_library', { pages: 304, language: 'Español' }),
  ]);
  assert.equal(corroborated.edition_fields_auto_publishable.pages, true);
