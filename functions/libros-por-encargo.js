...
function renderPage({ books, pageBooks, priorityBooks = [], currentPage, totalPages, indexable }) {
  ...
  return `<!doctype html><html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(pageTitle)}</title><meta name="description" content="${escapeHtml(pageDescription)}"><meta name="robots" content="${indexable ? 'index, follow' : 'noindex, nofollow'}"><link rel="canonical" href="${canonicalUrl}">${currentPage > 1 ? `<link rel="prev" href="${BASE}${orderHubPath(currentPage - 1)}">` : ''}${currentPage < totalPages ? `<link rel="next" href="${BASE}${orderHubPath(currentPage + 1)}">` : ''}${faviconHeadHtml()}${measurementHeadHtml()}<script type="application/ld+json">${JSON.stringify(schema).replace(/</g, '\\u003c')}</script><style>${styles}</style></head><body>
