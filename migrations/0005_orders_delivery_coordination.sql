-- CHECKOUT-TRANSFER-UX: la fecha y la franja se coordinan por WhatsApp
-- después del pago. Para crear una orden con envío sólo son obligatorios
-- dirección, localidad y departamento; los tres campos requested_* quedan
-- disponibles para compatibilidad con pedidos anteriores, pero son opcionales.
--
-- D1/SQLite no permite quitar un CHECK con ALTER TABLE. Se reconstruyen el
-- padre y sus dos tablas hijas con copia previa, siguiendo el procedimiento ya
-- usado y validado en 0002 para no perder order_items/order_events por CASCADE.
...
CREATE TABLE order_items_checkout_new (
  id                          TEXT    PRIMARY KEY,
  order_id                    TEXT    NOT NULL REFERENCES orders_checkout_new(id) ON DELETE CASCADE,
...
INSERT INTO order_items_checkout_new (
...
CREATE TABLE order_events_checkout_new (
  id           TEXT    PRIMARY KEY,
  order_id     TEXT    NOT NULL REFERENCES orders_checkout_new(id) ON DELETE CASCADE,
...
INSERT INTO order_events_checkout_new (id, order_id, event_type, payload_json, created_at)
...
ALTER TABLE order_items_checkout_new  RENAME TO order_items;
ALTER TABLE order_events_checkout_new RENAME TO order_events;
