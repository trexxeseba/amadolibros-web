...
export function checklyApi({ env = process.env, fetchFn = fetch } = {}) {
  ...
  return async (path, { method = 'GET', body } = {}) => {
    ...
    if (!response.ok) {
      // Sólo categorías cerradas; nunca imprimir el cuerpo o credenciales del proveedor.
      const error = await response.json().catch(() => ({}));
      const message = String(error.message || error.error || '').toLowerCase();
      const reason = /limit|quota|maximum|plan|upgrade/.test(message) ? '_PLAN_LIMIT' :
        /permission|authorized|forbidden|access/.test(message) ? '_PERMISSION' : '';
      throw new Error(`CHECKLY_${method}_HTTP_${response.status}${reason}`);
    }
