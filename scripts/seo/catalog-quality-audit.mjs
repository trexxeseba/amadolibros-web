...
export function auditCatalog(catalog, { source = null, generatedAt = new Date().toISOString() } = {}) {
  ...
  const coverage = {
    ...
    description: metric(active, item => nonEmpty(item.description)),
    language: metric(active, item => nonEmpty(item?.bibliographic?.language)),
    format: metric(active, item => nonEmpty(item?.bibliographic?.format)),
    edition: metric(active, item => nonEmpty(item?.bibliographic?.edition)),
    publication_year: metric(active, item => nonEmpty(item?.bibliographic?.publication_year)),
    genre: metric(active, item => nonEmpty(item?.bibliographic?.genre)),
  ...
  const priority = [
    ...
    ['description', 'SEO/CX: contexto real de ML para decidir la compra'],
    ['language', 'CX: evita comprar una edición en el idioma equivocado'],
    ['format', 'CX: aclara el formato físico del libro'],
    ['edition', 'SEO/CX: diferencia ediciones del mismo título'],
    ['publication_year', 'SEO/CX: ayuda a identificar la edición correcta'],
    ['genre', 'SEO/descubrimiento: mejora long-tail y comprensión'],
